...
def ping_check(config, params):
    ...
    endpoint = f"check-ping?host=check{params.get('host')}&max_nodes={params.get('max_nodes')}"
    return obj.make_api_call(endpoint=endpoint, health_check=True)

            
def http_check(config, params):
    obj = CheckHost(config)
    endpoint = f"check-http?host=check{params.get('host')}&max_nodes={params.get('max_nodes')}"
    return obj.make_api_call(endpoint=endpoint, health_check=True)


def tcp_connection_check(config, params):
    obj = CheckHost(config)
    endpoint = f"check-tcp?host=check{params.get('host')}&max_nodes={params.get('max_nodes')}"
    return obj.make_api_call(endpoint=endpoint, health_check=True)


def dns_address_check(config, params):
    obj = CheckHost(config)
    endpoint = f"check-dns?host=check{params.get('host')}&max_nodes={params.get('max_nodes')}"
    return obj.make_api_call(endpoint=endpoint, health_check=True)
